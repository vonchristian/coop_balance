  ...
  module BalanceFinders
    describe ToDate do
      it '.compute' do
        ...
        expect(described_class.new(amounts: AccountingModule::Amount.all, to_date: Date.current).compute).to be 800

        expect(described_class.new(amounts: AccountingModule::Amount.all, to_date: Date.current.last_year).compute).to be 600
      end
    end
  end
end
