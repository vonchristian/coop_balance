  ...
  module ScheduleCounters
    describe SemiAnnuallyCounter do
      it 'number of months per quarter' do
        expect(LoansModule::ScheduleCounters::SemiAnnuallyCounter::NUMBER_OF_MONTHS_PER_SEMI_ANNUAL).to be 6
      end
      ...
      it '#schedule_count' do
        ...
        expect(described_class.new(loan_application: loan_application_semi_annually_12).schedule_count).to be 2
        expect(described_class.new(loan_application: loan_application_semi_annually_48).schedule_count).to be 8
      end
    end
  end
end
