...
module LoansModule
  describe LoanApplication do
    ...
    describe 'delegations' do
      ...
      it { should delegate_method(:current_interest_config).to(:loan_product) }
      it { should delegate_method(:amortization_type).to(:loan_product) }

      it { should delegate_method(:avatar).to(:borrower).with_prefix }
      it { should delegate_method(:straight_balance?).to(:current_interest_config).with_prefix }
      it { should delegate_method(:annually?).to(:current_interest_config).with_prefix }
    end

    describe 'enums' do
      it { define_enum_for(:mode_of_payment).with_values(%i[daily weekly monthly semi_monthly quarterly semi_annually annually]) }
    end
