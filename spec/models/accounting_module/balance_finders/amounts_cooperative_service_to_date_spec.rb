...
module AccountingModule
  module BalanceFinders
    describe AmountsCooperativeServiceToDate do
      it '.compute' do
        ...
        entry = build(:entry, previous_entry: origin_entry, cooperative: cooperative, cooperative_service: cooperative_service, entry_date: Date.current.last_month)
        entry.debit_amounts.build(amount: 100, account: asset)
        entry.credit_amounts.build(amount: 100, account: revenue)
        entry.save!

        entry_2 = build(:entry, previous_entry: origin_entry, cooperative: cooperative, cooperative_service: cooperative_service, entry_date: Date.current)
