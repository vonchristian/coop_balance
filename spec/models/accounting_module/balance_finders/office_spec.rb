  ...
  module BalanceFinders
    describe Office do
      it '.compute' do
        ...
        entry        = build(:entry, previous_entry: origin_entry, cooperative: cooperative, office: office_1, entry_date: Date.current)
        entry.debit_amounts.build(amount: 100, account: asset)
        entry.credit_amounts.build(amount: 100, account: revenue)
        entry.save!

        entry_2 = build(:entry, previous_entry: entry, cooperative: cooperative, office: office_2, entry_date: Date.current.last_year)
