module Employees
  class ShareCapitalForm
    ...
    def subscribe_to_share_capital
      ...
      share_capital.capital_build_ups.capital_build_up.create!(entry_type: 'capital_build_up', recorder_id: recorder_id, description: 'Payment of capital build up', reference_number: reference_number, entry_date: date,
      debit_amounts_attributes: [account: debit_account, amount: amount, commercial_document: share_capital],
      credit_amounts_attributes: [account: credit_account, amount: amount, share_capital: share_capital])
    end
