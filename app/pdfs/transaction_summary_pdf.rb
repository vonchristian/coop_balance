...
class TransactionSummaryPdf < Prawn::Document
  ...
  def savings_deposits_balances
    [ [ "", "Beginning Balance", price(cooperative.saving_products.total_balance(to_date: date.yesterday.end_of_day)).to_s ] ]
  end

  def add_savings_deposits
    [ [ "", "", "Add Deposits", price(cooperative.saving_products.total_credits_balance(from_date: date, to_date: date)).to_s ] ]
  end

  def less_savings_deposits
    [ [ "", "", "Less Withdrawals", price(cooperative.saving_products.total_debits_balance(from_date: date, to_date: date)).to_s ] ]
  end

  def total_savings_deposits
    [ [ "", "", "Total Savings", price(cooperative.saving_products.total_balance(to_date: date)).to_s ] ]
  end
  ...
  def time_deposits_balances
    [ [ "", "Beginning Balance", price(cooperative.time_deposit_products.total_balance(to_date: date)).to_s ] ]
  end

  def time_deposits_from_members
    [ [ "", "", "Add Deposits", price(cooperative.time_deposit_products.total_credits_balance(from_date: date, to_date: date)).to_s ] ]
  end

  def time_deposit_withdrawals_from_members
    [ [ "", "", "Less Withdrawals", price(cooperative.time_deposit_products.total_debits_balance(from_date: date, to_date: date)).to_s ] ]
  end

  def total_time_deposits
    [ [ "", "", "Total Time Deposits", price(cooperative.time_deposit_products.total_balance(to_date: date)).to_s ] ]
  end
  ...
  def share_capital_beginning_balance
    [ [ "", "Beginning Balance", price(cooperative.share_capital_products.total_balance(to_date: date.yesterday.end_of_day)).to_s ] ]
  end

  def additional_share_capital
    [ [ "", "", "Add Share Capital", price(cooperative.share_capital_products.total_credits_balance(from_date: date, to_date: date)).to_s ] ]
  end

  def total_share_capitals
    [ [ "", "", "Total Share Capitals", price(cooperative.share_capital_products.total_balance(to_date: date)).to_s ] ]
  end

  def share_capital_withdrawals
    [ [ "", "", "Less Withdrawals", price(cooperative.share_capital_products.total_debits_balance(from_date: date, to_date: date)).to_s ] ]
  end
  ...
  def loan_collections
    ...
    cooperative.entries.loan_payments.recorded_by(recorder: employee).entered_on(from_date: date, to_date: date).each do |entry|
      table(
        cooperative.loans.for_entry(entry: entry).map { |loan|
          [
            loan.borrower_name.to_s,
            loan.loan_product_name.to_s,
            ...
            price(entry.credit_amounts.total_loan_payment(loan: loan))
          ]
        }, column_widths: [ 80, 80, 60, 70, 70, 70, 80 ], cell_style: { inline_format: true, size: 9, padding: [ 0, 0, 0, 0 ] }
      ) do
        cells.borders = []
        column(3).align = :right
        column(4).align = :right
    ...
    table([ [ "", "", "TOTAL",
            price(cooperative.loan_products.total_credits_balance(from_date: date, to_date: date)).to_s,
            price(cooperative.loan_products.interest_revenue_accounts.credits_balance(from_date: date, to_date: date)).to_s,
            price(cooperative.loan_products.penalty_revenue_accounts.credits_balance(from_date: date, to_date: date)).to_s,
            price(cooperative.entries.loan_payments(from_date: date, to_date: date)).to_s ] ], column_widths: [ 80, 80, 60, 70, 70, 70, 80 ], cell_style: { inline_format: true, size: 9, padding: [ 0, 0, 0, 0 ] }) do
      cells.borders = []
      column(3).align = :right
      column(4).align = :right
      column(5).align = :right
  ...
  def accounts_total_data
    [ [ "",
      "",
      "",
      price(cooperative.accounts.updated_at(from_date: date, to_date: date).updated_by(employee).debits_balance(from_date: date, to_date: date)).to_s,
      price(cooperative.accounts.updated_at(from_date: date, to_date: date).updated_by(employee).credits_balance(from_date: date, to_date: date)).to_s ] ]
  end
end
