...
module StatementOfAccounts
  class SavingsAccountPdf < Prawn::Document
    ACCOUNT_SUMMARY_TABLE_WITDHS = [ 10, 120, 200 ].freeze
    ...
    def savings_account_details
      ...
      table([ [ "", "Depositor", savings_account.depositor_name.to_s ] ], cell_style: { padding: [ 0, 0, 2, 0 ], inline_format: true, size: 10 }, column_widths: ACCOUNT_SUMMARY_TABLE_WITDHS) do
        cells.borders = []
      end
      table([ [ "", "Savings Product", savings_account.saving_product_name.to_s ] ], cell_style: { padding: [ 0, 0, 2, 0 ], inline_format: true, size: 10 }, column_widths: ACCOUNT_SUMMARY_TABLE_WITDHS) do
        cells.borders = []
      end
      table([ [ "", "Last Transaction", savings_account.last_transaction_date.strftime("%B %e, %Y").to_s ] ], cell_style: { padding: [ 0, 0, 2, 0 ], inline_format: true, size: 10 }, column_widths: ACCOUNT_SUMMARY_TABLE_WITDHS) do
        cells.borders = []
      end
      table([ [ "", "Days Inactive", savings_account.number_of_days_inactive.to_s ] ], cell_style: { padding: [ 0, 0, 2, 0 ], inline_format: true, size: 10 }, column_widths: ACCOUNT_SUMMARY_TABLE_WITDHS) do
        cells.borders = []
      end
      bounding_box [ 280, 690 ], width: 400 do
        ...
        move_down 5
        text price(savings_account.balance).to_s, size: 18
      end
