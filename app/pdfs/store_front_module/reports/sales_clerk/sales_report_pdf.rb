      ...
      class SalesReportPdf < Prawn::Document
        ...
        def summary
          bounding_box [0, 700], width: 200 do
            text 'SUMMARY:'
            table([['CASH SALES', @sales_orders.cash_sales.sum(&:total_cost).to_s]], header: true, cell_style: { size: 10, font: 'Helvetica' }, column_widths: [100, 100]) do
              cells.borders = []
              column(1).font_style = :bold
            end
            table([['CREDIT SALES', @sales_orders.credit_sales.sum(&:total_cost).to_s]], header: true, cell_style: { size: 10, font: 'Helvetica' }, column_widths: [100, 100]) do
              cells.borders = []
              column(1).font_style = :bold
            end
            table([['TOTAL SALES', @sales_orders.total.to_s]], header: true, cell_style: { size: 10, font: 'Helvetica' }, column_widths: [100, 100]) do
              cells.borders = []
              column(1).font_style = :bold
            end
            table([['INCOME', price(@sales_orders.total_income).to_s]], header: true, cell_style: { size: 10, font: 'Helvetica' }, column_widths: [100, 100]) do
              cells.borders = []
              column(1).font_style = :bold
            end
          end
          bounding_box [300, 700], width: 235 do
            ...
            table([['Less Remittances', @sales_orders.total.to_s]], header: true, cell_style: { size: 10, font: 'Helvetica' }, column_widths: [100, 100]) do
              cells.borders = []
              column(1).font_style = :bold
            end
            stroke do
