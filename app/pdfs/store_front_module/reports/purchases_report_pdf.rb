    ...
    class PurchasesReportPdf < Prawn::Document
      ...
      def summary
        bounding_box [0, 700], width: 200 do
          text "SUMMARY:"
          table([["CASH PURCHASES", "#{price(@purchase_orders.processed.sum(&:total_cost))}"]], header: true, cell_style: { size: 14, font: "Helvetica"}, column_widths: [100, 100]) do
            cells.borders = []
        ...
        def purchase_details
          @purchase_orders.processed.each do |purchase_order|
            if purchase_order.line_items.present?
              table([["#{purchase_order.date.try(:strftime, ("%B %e, %Y"))}", "#{purchase_order.supplier_name.try(:upcase)}"]]) do
                cells.borders = []
              end
              ...
              table([["", "", "", "TOTAL", "#{purchase_order.total_cost}"]], cell_style: { size: 10, font: "Helvetica"}, column_widths: [100, 50, 150, 100, 100]) do
                column(3).align = :right
                column(2).align = :right
              end
            end
