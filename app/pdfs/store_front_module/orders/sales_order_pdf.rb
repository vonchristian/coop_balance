  ...
  module Orders
    class SalesOrderPdf < Prawn::Document
      ...
      def customer_details
        table([["CUSTOMER", "#{@sales_order.customer_name.try(:upcase)}"]], header: true, cell_style: { size: 6, font: "Helvetica"}, column_widths: [100, 100]) do
          cells.borders = []
          column(1).font_style = :bold
        end
        table([["PURCHASES (#{Date.today.year})", "#{price(@sales_order.commercial_document.total_purchases(fron_date: Date.today.beginning_of_year, to_date: Date.today.end_of_year))}"]], header: true, cell_style: { size: 6, font: "Helvetica"}, column_widths: [100, 100]) do
          cells.borders = []
          column(1).font_style = :bold
        end
        table([["TOTAL CREDIT", "#{price(@sales_order.commercial_document.total_purchases)}"]], header: true, cell_style: { size: 6, font: "Helvetica"}, column_widths: [100, 100]) do
          cells.borders = []
        end
      end
      ...
      def cashier_details
        ...
        text "#{@sales_order.date.strftime("%b, %e, %Y %H:%m %p")}"
