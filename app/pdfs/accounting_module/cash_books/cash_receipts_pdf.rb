module AccountingModule
  module CashBooks
    class CashReceiptsPdf < Prawn::Document
          ...
          entries_data ||= entries.sort_by{|e| e.reference_number.to_i}.map do |entry|
            if entry.cancelled?
              ...
              [
                ...
                entry.debit_amounts.where(account: office.cash_accounts).sum{|a| a.amount}.zero? ? "-" : price(entry.debit_amounts.where(account: office.cash_accounts).sum{|a| a.amount}), 
      ...
      def sub_totals_table
        if grouped_similar_reference_numbers.any?
          ...
          sub_total_data ||= grouped_similar_reference_numbers.sort_by{|e| e.to_i}.map do |reference_number|
            ["", "", "No. of Entries: #{entries.where(reference_number: reference_number).count}", "##{reference_number}", "#{price(debits_sub_total(reference_number))}", ""]
          end
      ...
      def grand_total
        AccountingModule::DebitAmount.where(entry_id: entries.pluck(:id)).where(account: office.cash_accounts).sum{|a| a.amount}
      end
