    ...
    class CashDisbursementsPdf < Prawn::Document
      ...
      def entries_table
        if @entries.none?
          ...
          entries_data ||= entries.sort_by(&:ascending_order).map do |entry|
            if entry.cancelled?
      ...
      def grand_total
        AccountingModule::CreditAmount.where(entry_id: entries.pluck(:id)).where(account: office.cash_accounts).sum(&:amount)
      end
      ...
      def debits_sub_total(reference_number)
        ...
        AccountingModule::DebitAmount.where(entry_id: ids).where(account: office.cash_accounts).sum(&:amount)
      end

      def credits_sub_total(reference_number)
        ids = entries.where(reference_number: reference_number).pluck(:id)
        AccountingModule::CreditAmount.where(entry_id: ids).where(account: office.cash_accounts).sum(&:amount)
      end
    end
  end
end
