module AccountingModule 
  class CollectionReportPdf < Prawn::Document 
    ...
    def accounts_data
      ...
      [["Cash Receipts", "#{price(@employee.cash_on_hand_account.debits_balance(from_date: (@from_date.beginning_of_day - 1.second), to_date: @to_date.end_of_day, recorder_id: @employee.id))}"]] +
      [["Current Balance", "#{price(@employee.cash_on_hand_account.balance(recorder_id: @employee.id) + @employee.fund_transfer_total)}" ]]
    end
    def entries_table
    if !@entries.any?
      move_down 10
