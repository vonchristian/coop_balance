  ...
  class LoanAmortizationSchedulePdf < Prawn::Document
    ...
    def loan_details
      loan_details_data ||= [[{ content: 'LOAN DETAILS', size: 9 }, '']] +
                            [['Loan Product', loan.loan_product_name.to_s]] +
                            [['Loan Amount ', price(loan.loan_amount).to_s]] +
                            [['Amount (in words)', "#{loan.loan_amount.to_f.to_words.titleize} Pesos"]] +
                            [['Term ', "#{term} Month/s"]] +
                            [['Disbursement Date ', loan.disbursement_date.strftime('%B %e, %Y').to_s]] +
                            [['Maturity Date ', loan.maturity_date.strftime('%B %e, %Y').to_s]]
    ...
    def signatory
      [['PREPARED BY', '', 'APPROVED BY', '', 'DISBURSED BY', '', 'RECIEVED BY']] +
        ...
        [['', '', '', '', '', '', '']] +
        [['', '', '', '', '', '', '']] +
        [[preparer.first_middle_and_last_name.try(:upcase).to_s, '',
          approver.first_middle_and_last_name.try(:upcase), '',
          disburser.first_middle_and_last_name.upcase, '',
          @loan.borrower.first_middle_and_last_name.try(:upcase).to_s]] +
        [[preparer.designation.try(:titleize).to_s, '',
          approver.designation.try(:titleize).to_s, '',
          disburser.designation.try(:titleize).to_s, '',
          'Borrower']]
    end
  end
end
