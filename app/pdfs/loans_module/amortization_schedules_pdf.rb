module LoansModule
  class AmortizationSchedulesPdf < Prawn::Document
    ...
    def summary
      ...
      table([ [ "# of Amortization", amortization_schedules.count.to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
        cells.borders = []
        column(1).align = :right
      end
      move_down 2
      table([ [ "TOTAL PRINCIPAL", price(amortization_schedules.sum(&:principal)).to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
        cells.borders = []
        column(1).align = :right
      end
      move_down 2
      table([ [ "TOTAL INTEREST", price(amortization_schedules.sum(&:interest)).to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
        cells.borders = []
        column(1).align = :right
      end
      move_down 2
      table([ [ "TOTAL AMORTIZATION", price(amortization_schedules.sum(&:total_amortization)).to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
        cells.borders = []
        column(1).align = :right
      end
