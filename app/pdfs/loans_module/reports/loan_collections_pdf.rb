  ...
  module Reports
    class LoanCollectionsPdf < Prawn::Document
      ...
      def summary
        ...
        table([["Total Interest", ""]], cell_style: {padding: [0,0,0,0], inline_format: true, size: 12}, column_widths: [120, 100]) do
          cells.borders = []
          column(1).align = :right
        end
        move_down 5
        ...
        table([["Total Penalty", ""]], cell_style: {padding: [0,0,0,0], inline_format: true, size: 12}, column_widths: [120, 100]) do
          cells.borders = []
          column(1).align = :right
        ...
        table([["Total Collection", ""]], cell_style: {padding: [0,0,0,0], inline_format: true, size: 12}, column_widths: [120, 100]) do
          cells.borders = []
          column(1).align = :right
      ...
      def loan_collections
       ...
       # if cooperative.entries.loan_payments.recorded_by(recorder: employee).entered_on(from_date: date, to_date: date).present?
         table([["Borrower", "LOAN", "OR #", "Principal", "Interest", "Penalty", "Total"]], header: true, cell_style: { inline_format: true, size: 10, font: "Helvetica"}, column_widths: [80, 80, 60, 70, 70, 70, 80]) do
           cells.borders = []
       ...
       collections.each do |entry|
        ...
        table([["", "", "TOTAL",
          "#{price(cooperative.loan_products.total_credits_balance(from_date: from_date, to_date: to_date))}",
          ...
          "#{price(cooperative.loan_products.loan_payments(from_date: from_date, to_date: to_date))}"
