    ...
    class LoanCollectionsPdf < Prawn::Document
      ...
      def summary
        ...
        if loan_product.present?
          table([ [ "Loan Product", loan_product.name.upcase.to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 100, 200 ]) do
            cells.borders = []
        ...
        table([ [ "Collections Count", collections.count.to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
          cells.borders = []
        ...
        table([ [ "Total Principal",
                price(LoansModule::Payments::Aggregator.new(loan_product: loan_product, collections: collections, from_date: from_date, to_date: to_date, cooperative: cooperative).total_principals).to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
          cells.borders = []
          column(1).align = :right
        end
        move_down 5

        table([ [ "Total Interest", price(LoansModule::Payments::Aggregator.new(loan_product: loan_product, collections: collections, from_date: from_date, to_date: to_date, cooperative: cooperative).total_interests).to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
          cells.borders = []
          column(1).align = :right
        end
        move_down 5

        table([ [ "Total Penalty", price(LoansModule::Payments::Aggregator.new(loan_product: loan_product, collections: collections, from_date: from_date, to_date: to_date, cooperative: cooperative).total_penalties).to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
          cells.borders = []
          column(1).align = :right
        end
        move_down 5
        ...
        move_down 5
        table([ [ "Total Collections", price(LoansModule::Payments::Aggregator.new(loan_product: loan_product, collections: collections, from_date: from_date, to_date: to_date, cooperative: cooperative).total_cash_payments).to_s ] ], cell_style: { padding: [ 0, 0, 0, 0 ], inline_format: true, size: 10 }, column_widths: [ 120, 100 ]) do
          cells.borders = []
          column(1).align = :right
        end
        move_down 10
        ...
        if collections.present?
          ...
          collections.sort_by(&:ascending_order).each do |entry|
            cooperative.loans.for_entry(entry: entry).each do |loan|
              table(
                [ [ entry.entry_date.strftime("%D").to_s,
                  ...
                  price(LoansModule::Payments::Classifier.new(loan: loan, entry: entry).total_cash_payment) ] ], column_widths: [ 50, 110, 50, 60, 60, 60, 60, 80 ], cell_style: { padding: [ 2, 3, 2, 3 ], inline_format: true, size: 9 }
              ) do
