module LoansModule
    ...
    def self.create_lumpsum_amortization_schedule(loan)
      loan.amortization_schedules.create(date: starting_date(loan) + loan.term.to_i.months, principal: (loan.loan_amount), interest: loan.interest_on_loan_balance)
    end
    def self.create_quarterly_amortization_schedule(loan)
      first_amortization = loan.amortization_schedules.create(date: starting_date(loan).next_quarter, principal: (loan.loan_amount / 4), interest: (loan.interest_on_loan_balance / 4))
      ActiveRecord::Base.transaction do
    ...
    def self.create_semi_annuall_amortization_schedule(loan)
      first_amortization = loan.amortization_schedules.create(date: starting_date(loan).next_quarter.next_quarter, principal: (loan.loan_amount / (loan.term.to_i/6)), interest: (loan.interest_on_loan_balance/(loan.term.to_i/6)))
        ActiveRecord::Base.transaction do
