   ...
   def self.disbursed_on(hash={})
    ...
    def add_amounts_from(loan)
      ...
      loan.loan_charges.each do |charge|
        self.voucher_amounts.create(amount_type: 'credit', amount: charge.charge_amount_with_adjustment, description: charge.name, account_id: charge.account.id, commercial_document: charge)
      end
    end
  end
end
