  ...
  class MemberRegistry < Registry
    def parse_for_records
      ...
      (2..member_spreadsheet.last_row).each do |i|
        row = [ header, member_spreadsheet.row(i) ].transpose.to_h
        ActiveRecord::Base.transaction do
          Member.where(first_name: row["First Name"], last_name: row["Last Name"]).first_or_create! do |m|
