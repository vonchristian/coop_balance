      ...
      (2..member_spreadsheet.last_row).each do |i|
        ...
        ActiveRecord::Base.transaction do 
          member = Member.where(first_name: row['First Name'], last_name: row['Last Name'], middle_name: row['Middle Name']).first_or_create! do |m|
            ...
            Membership.create!(account_number: row["Account Number"], cooperator: m, membership_date: membership_date(row), cooperative: Cooperative.find_by(name: row["Cooperative"]))
            m.save        
          end
        end
      end
