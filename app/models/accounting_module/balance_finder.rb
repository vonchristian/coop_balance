  ...
  module BalanceFinder
    def balance(hash={})
      ...
      if commercial_document.present? && from_date.present? && to_date.present?
        ...
        date_range = DateRange.new(from_date: from_date, to_date: to_date)
        joins(:entry, :account).where('entries.entry_date' => date_range.range).sum(:amount)
      elsif commercial_document.present? && from_date.blank? && to_date.blank?
        joins(:entry, :account).where('commercial_document_id' => commercial_document).sum(:amount)
