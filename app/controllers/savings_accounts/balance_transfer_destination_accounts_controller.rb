module SavingsAccounts
  class BalanceTransferDestinationAccountsController < ApplicationController
    def new
      @origin_saving = current_cooperative.savings.find(params[:origin_saving_id])
      @destination_saving = SavingsAccounts::BalanceTransferDestinationAccountProcessing.new
      if params[:search].present?
        @savings_accounts = current_cooperative.savings.text_search(params[:search]).paginate(page: params[:page], per_page: 25)
      else
