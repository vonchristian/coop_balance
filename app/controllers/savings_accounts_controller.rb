class SavingsAccountsController < ApplicationController
  def index
    if params[:search].present?
      ...
      @savings_accounts = MembershipsModule::Saving.includes([:depositor, :saving_product, :entries]).order(:account_owner_name).all.paginate(:page => params[:page], :per_page => 20)
    end
  end
