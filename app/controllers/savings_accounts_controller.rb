class SavingsAccountsController < ApplicationController
  def index
    if params[:search].present?
      ...
      @savings_accounts = MembershipsModule::Saving.includes([:depositor, :saving_product =>[:account, :interest_account], :entries =>[:amounts]]).order(:account_owner_name).all.paginate(:page => params[:page], :per_page => 20)
    end
  end
