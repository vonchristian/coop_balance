module StoreFrontModule
  module LineItems
    class PurchaseLineItemsController < ApplicationController
      def new
        @products = StoreFrontModule::Product.text_search(params[:search]).all.paginate(page: params[:page], per_page: 30)
        @line_items = StoreFrontModule::LineItems::PurchaseLineItem.text_search(params[:search]).paginate(page: params[:page], per_page: 30)
        @cart = current_cart
