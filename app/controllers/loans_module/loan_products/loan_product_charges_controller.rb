  ...
  module LoanProducts
    class LoanProductChargesController < ApplicationController
      ...
      def loan_product_charge_params
        params.require(:loans_module_loan_products_loan_product_charge).permit(charge_attributes: [:name, :charge_type, :percent, :amount, :account_id, :minimum_loan_amount, :maximum_loan_amount, :depends_on_loan_amount])
      end
    end
  end
end
