module LoansModule
  class MaturedLoansController < ApplicationController
    def index
      @from_date = params[:from_date] ? DateTime.parse(params[:from_date]) : Time.zone.today.to_date
      @to_date = params[:to_date] ? DateTime.parse(params[:to_date]) : Time.zone.today.to_date
