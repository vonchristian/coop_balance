    ...
    class LoansController < ApplicationController
      def new
        if params[:search].present?
          @pagy_loans, @loans           = pagy(current_office.loans.includes(:loan_product, :receivable_account, borrower: [avatar_attachment: [:blob]]).text_search(params[:search]))
        else
          @pagy_loans, @loans           = pagy(current_office.loans.includes(:loan_product, :receivable_account, borrower: [avatar_attachment: [:blob]]))
        end
