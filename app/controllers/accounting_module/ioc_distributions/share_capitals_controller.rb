    ...
    class ShareCapitalsController < ApplicationController
      def new
        if params[:search].present?
          ...
          @pagy_share_capitals, @share_capitals = pagy(current_office.share_capitals.includes(:share_capital_product, share_capital_equity_account: %i[debit_amounts credit_amounts], subscriber: [ avatar_attachment: [ :blob ] ]))
        end
