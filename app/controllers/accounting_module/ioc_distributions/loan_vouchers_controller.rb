module AccountingModule
  module IocDistributions
    class LoanVouchersController < ApplicationController
      def create 
        @voucher = AccountingModule::IocDistributions::LoanVoucher.new(voucher_params)
        if @voucher.valid?
          @voucher.process!
          @find_voucher = current_office.vouchers.find_by(account_number: params[:accounting_module_ioc_distributions_loan_voucher][:account_number])
      ...
      private 
      def voucher_params
        params.require(:accounting_module_ioc_distributions_loan_voucher).
