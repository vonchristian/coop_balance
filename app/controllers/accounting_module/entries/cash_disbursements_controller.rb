    ...
    class CashDisbursementsController < ApplicationController
      def index
          ...
          if params[:search].present?
            ...
            @pagy, @entries  = pagy(current_office.cash_accounts.credit_entries.includes(:commercial_document, :debit_amounts).order(reference_number: :desc).entered_on(from_date: @from_date, to_date: @to_date))
          end
