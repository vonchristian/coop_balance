    ...
    class CashDisbursementsController < ApplicationController
      def index
        ...
        if params[:from_date].present? && params[:to_date].present?
          ...
          @ordered_entries = current_cooperative.cash_accounts.credit_entries.order(reference_number: :desc).where(cooperative_service_id: params[:cooperative_service_id])
          @entries = @ordered_entries.paginate(page: params[:page], per_page:  50)
        end
