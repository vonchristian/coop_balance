    ...
    class CashDisbursementsController < ApplicationController
      def index
          ...
          if params[:search].present?
            ...
            @pagy, @entries  = pagy(current_office.cash_accounts.credit_entries.order(reference_number: :desc).text_search(params[:search]))
          else 
            @entries_for_pdf = current_office.cash_accounts.credit_entries.order(reference_number: :desc).entered_on(from_date: @from_date, to_date: @to_date)
            @pagy, @entries  = pagy(current_office.cash_accounts.credit_entries.order(reference_number: :desc).entered_on(from_date: @from_date, to_date: @to_date))
          end
