  ...
  class AccountBudgetsController < ApplicationController
    ...
    def new
      if params[:search].present?
        @accounts = current_cooperative.accounts.text_search(params[:search]).paginate(page: params[:page], per_page: 25)
      else
        @accounts = current_cooperative.accounts.all.paginate(page: params[:page], per_page: 25)
      end
