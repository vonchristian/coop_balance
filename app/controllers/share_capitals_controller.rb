...
class ShareCapitalsController < ApplicationController
  ...
  def show
    ...
    @entries = @share_capital.entries.order(entry_date: :desc).includes(:commercial_document, :recorder, :cooperative_service).sort_by(&:entry_date).reverse.paginate(page: params[:page], per_page: 25)
