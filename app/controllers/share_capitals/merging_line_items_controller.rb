  ...
  class MergingLineItemsController < ApplicationController
    def new
      ...
      if params[:search].present?
        @share_capitals = MembershipsModule::ShareCapital.where.not(id: @current_share_capital.id).text_search(params[:search]).paginate(page: params[:page], per_page: 25)
      else
        @share_capitals = MembershipsModule::ShareCapital.where.not(id: @current_share_capital.id).text_search(@current_share_capital.subscriber_name).paginate(page: params[:page], per_page: 25)
      end
    end
    def create
      @current_share_capital = MembershipsModule::ShareCapital.find(params[:share_capital_id])
