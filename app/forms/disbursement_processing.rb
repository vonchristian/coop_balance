class DisbursementProcessing
  ...
  def find_payee
    employee_payee = User.find_by_id(payee_id)
    supplier_payee = Supplier.find_by_id(payee_id)
    member_payee = Member.find_by_id(payee_id)
    if employee_payee.present?
      employee_payee
    elsif supplier_payee.present?
      supplier_payee
    elsif member_payee.present?
      member_payee
    end
  end
